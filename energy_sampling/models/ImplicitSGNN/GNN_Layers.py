...
class GBNeck_interaction(MessagePassing):
    ...
    def createRadiusToIndexMap(self):
        self._radiusToIndex = {r: i for i, r in enumerate(self._uniqueRadii)}

    def createUniqueTable(self, fullTable):
        tablePositions = [(r+self._OFFSET-0.1)*200 for r in self._uniqueRadii]
    ...
    def getm0(self,idx1: torch.Tensor,idx2: torch.Tensor) ->  torch.Tensor:
        indices = self._num_unique * idx2 + idx1
        return torch.index_select(self._m0,0,indices)

    def getd0(self,idx1:torch.Tensor,idx2: torch.Tensor) ->  torch.Tensor:
        indices = self._num_unique * idx2 + idx1
    ...
    def forward(self, edge_index, x, edge_attributes):
        ...
        # propagate_type: (I: torch.Tensor, x: torch.Tensor)
        B = self.nodewise(I,x)

        return B

    def nodewise(self,I : torch.Tensor ,x : torch.Tensor) -> torch.Tensor:
        ...
        offset = torch.tensor(0.0195141,dtype=torch.float,device=self._device)
        radius = x[:,1] + offset
        psi = I * x[:,1]
        B = 1/(1/x[:,1]-torch.tanh(alpha*psi-beta*psi**2+gamma*psi**3)/radius)
        Bc = torch.concat((B.unsqueeze(1),x[:,0].unsqueeze(1)),dim=1)
        return Bc
    ...
    def message(self, x_i : torch.Tensor, x_j : torch.Tensor , edge_attributes : torch.Tensor) -> torch.Tensor:
        ...
        Ivdw = torch.where((r + sr2 - or1) > 0, 0.5*(1/L-1/U+0.25*(r-sr2**2/r)*(1/(U**2)-1/(L**2))+0.5*torch.log(L/U)/r),
                           torch.tensor(0, dtype=torch.float,device=self._device))

        Ineck = torch.where((radius1 + radius2 + neckCut - r) > 0, self.getm0(radindex1, radindex2) / (
                    1 + 100 * (r - self.getd0(radindex1, radindex2)) ** 2 + 0.3 * 1000000 * (
                        r - self.getd0(radindex1, radindex2)) ** 6),torch.tensor(0, dtype=torch.float,device=self._device))
...
class IN_layer_all_swish_2pass(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attributes):
        ...
        # propagate_type: (x: torch.Tensor, edge_attributes: torch.Tensor)
        x = self.propagate(edge_index, x=x, edge_attributes=edge_attributes,size=None)
